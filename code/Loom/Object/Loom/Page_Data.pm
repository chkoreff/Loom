package Loom::Object::Loom::Page_Data;
use strict;
...
sub respond
	...
	my $response = $s->data($op->get("name"));
...
sub data
	...
	if ($name eq "style.css")
		{
		return $s->data_style_sheet;
		}
	elsif ($name eq "keyboard.png")
		{
		return $s->data_keyboard_png;
		}
	elsif ($name eq "keyboard.css")
		{
		return $s->data_keyboard_css;
		}
	elsif ($name eq "keyboard.js")
		{
		return $s->data_keyboard_js;
		}
...
A.contact:link, A.contact:visited { color:black; text-decoration:underline }
A.contact:hover { color:gray; text-decoration:underline }
A.contact:active { color:#FAD805; text-decoration:underline }

td.folder_section { font-weight:bold; font-size:10pt; padding-top:10px; border-bottom: solid 1px; }
td.folder_section_links { padding-top:10px; border-bottom: solid 1px; }
EOM
	}

sub data_keyboard_png
	{
	my $s = shift;
...
EOM
	...
	$data =~ s/keyboard.png/$url_keyboard_png/g;
