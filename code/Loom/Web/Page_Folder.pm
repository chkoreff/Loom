...
sub new
	...
	$s->{module} =
		{
		folder => "Loom::Web::Page_Wallet",
		contact => "Loom::Web::Page_Contact",
		asset => "Loom::Web::Page_Asset",
		};
...
EOM
	...
	push @{$site->{top_links}},
		$site->highlight_link(
			...
			"Contacts",
			$op->get("function") eq "contact" && $op->get("help") eq "",
...
EOM
	...
	if ($display->{move_dialog} ne "")
		...
		if ($error eq "")
			...
			{
			$error = qq{You already have that one in your wallet.};
			}
...
sub page_login
	...
	if ($op->get("help"))
		...
		# Strange but oh well.
		$site->object("Loom::Web::Page_Help",$site,"index")->respond;
		return;
		}
...
sub build_folder_template
	...
	{
	my $cipher = Loom::Crypt::Span->new("\000"x16);
	my $blocks = $cipher->encrypt($build->{folder_text});
