...
EOM
	...
	if ($op->get("edit_history_items") eq "")
	{
	$table .= <<EOM;
<tr>
...
EOM
	...
	my $is_checked = $op->get("choose_$h_id") ne "" ? 1 : 0;
...
EOM
	...
	if ($is_checked && $op->get("edit_history_items") ne "")
	{
...
EOM
	...
	if ($first_memo ne "")
		{
		$site->set_focus("memo_$first_memo");
		}
...
sub handle_history
	...
	if ($op->get("delete_history_items") ne "")
		{
		for my $name ($op->names)
			{
			next unless $name =~ /^choose_(.+)$/;
			my $h_id = $1;
			$s->delete_history_item($h_id);
			}
	...
	if ($op->get("save_history_items") ne "")
		{
		for my $name ($op->names)
			{
			next unless $name =~ /^choose_(.+)$/;
			my $h_id = $1;
			$s->save_history_item($h_id,$op->get("memo_$h_id"));
			}
	...
	if ($op->get("cancel_history_items") ne "")
		{
		# Do nothing.
		return;
		}
...
sub save_history_item
	...
	$folder->{object}->put("H_memo.$h_id",$memo);
